...
connectionEndpointIDs:
        d3.map(),
heapConnectionEndpointIDs:
        d3.map(), // subset of connectionEndpointIDs for heap->heap connections
        // analogous to connectionEndpointIDs, except for environment parent pointers
parentPointerConnectionEndpointIDs:
        d3.map(),

renderedHeapObjectIDs:
        d3.map(), // format given by generateHeapObjID()
    };
}
...
ExecutionVisualizer.prototype.render = function()
{
    ...
    var codeDisplayHTML =
       ...
       <span id="liveModeSpan" style="display: none;">| </a>\
       </div>\
       <div id="legendDiv"/>\
\
...
sliderDiv.slider( {min: 0, max:
                       this.curTrace.length - 1, step: 1});
...
this.try_hook("end_render", {myViz:
                                 this});
...
myViz.jsPlumbInstance.select( {source: labelID}).detach();
                   ...
                   .attr('class', function(d, i)
    {
        return d.is_zombie ? 'zombieStackFrame' : 'stackFrame';
                   ...
                   .attr('id', function(d, i)
    {
        return d.is_zombie ? myViz.generateID("zombie_stack" + i)
...
myViz.try_hook("end_renderDataStructures", {myViz:
                   myViz});
}
...
show:
    {
ready:
        true, // show on document.ready instead of on mouseenter
        delay: 0,
event:
        null,
effect:
